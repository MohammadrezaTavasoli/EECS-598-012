...
class ConvNet(object):
  ...
  batch normalize - conv - relu - 2x2 max pool - dropout mask - fc - softmax
  ...
  def __init__(self, input_dim=(1, 28, 28), num_filters=32, filter_size=7,
               ...
               dtype=np.float32, gamma = 1.0, beta = 1.0, bn_param = {}, dropout_param = {}):
    """
  ...
  def loss(self, X, y=None):
    ...
    batch_out, cache_batch = batchnorm_forward(X, self.gamma, self.beta, self.bn_param)
    out_1, cache_1 = conv_forward(batch_out, W1)
    out_2, cache_2 = relu_forward(out_1)
    out_3, cache_3 = max_pool_forward(out_2, pool_param)
    out_dropout, cache_dropout = dropout_forward(out_3, self.dropout_param)
    out_4, cache_4 = fc_forward(out_dropout, W2, b2)
    ...
    dx_dropout = dropout_backward(dx_4, cache_dropout)
    dx_3 = max_pool_backward(dx_dropout, cache_3)
    dx_2 = relu_backward(dx_3, cache_2)
    dx_1, grads['W1'] = conv_backward(dx_2, cache_1)
    dbatch_out, dgamma, dbeta = batchnorm_backward(dx_1, cache_batch)
    ...
    return loss, grads, dgamma, dbeta
